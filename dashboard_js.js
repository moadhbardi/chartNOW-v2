...
function renderDashboardChart(config, index) {
  const canvas = document.getElementById(`dashboardChart-${index}`);
  if (!canvas) {
    console.error(`Canvas dashboardChart-${index} not found`);
    return;
  }
  ...
  if (hasChartData) {
    ...
    const chartConfig = {
      ...
      options: {
        ...
        scales: config.chartOptions?.scales || {},
      },
    };
    ...
    console.log(`✅ Rendered ${config.type} chart from saved data`);
  } else {
      ...
      options: {
        ...
        scales: {
          x: {
            ...
            title: { display: true, text: config.xAxis || "X Axis" },
          },
          y: {
            stacked: config.type === "stackedBar",
            beginAtZero: true,
